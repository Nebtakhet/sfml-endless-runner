...
void Player::update(float dt)
{
	...
	const float worldWidth = 1024.0f; // Assuming fixed world width for boundary checks
	const float groundY = Entity::getGroundY() - 25.0f; // Added a small offset for better visual alignment
...
std::unique_ptr<Fireball> Player::releaseFireball()
{
	...
	m_charge = 0.0f; // reset charge

	return fb;
}

// Handle input events. Called from the main event loop. Player moves and does actions based on key presses.
